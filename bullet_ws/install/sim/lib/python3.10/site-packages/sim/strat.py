...
class SkillLib:
    def moveToPointOrg(self, robot: ObjData, target_pos: np.ndarray, target_theta: float, obstacles: list[np.ndarray], bypass = False) -> LowCmd:
    ...
    def moveToStrikeOrg(self, robot: ObjData, target_pos: np.ndarray, target_theta: float, obstacles: list[np.ndarray], strike_dir: np.ndarray) -> LowCmd:
        robot_pos = np.array([robot.x, robot.y])
    ...
    def _calculate_potential_field_vector_org(self, robot_pos: np.ndarray, robot_vel: np.ndarray, target_pos: np.ndarray, obstacles: list[np.ndarray], bypass = False) -> np.ndarray:
...
class Strat(Node):
    ...
    def isInRegion(self, obj, region):
        return region[0] <= obj[0] <= region[2] and region[1] <= obj[1] <= region[3]
                ...
                for i, team_index in enumerate(self.mapping):
                    ...
                    if team_index == attacker:
                        ...
                        if ball_coords[1] > FIELD_LIMS[1] - 2 or ball_coords[1] < -FIELD_LIMS[1] + 2:
                            print("top/bottom margins")
                            self.cmds[i], pos = self.skills.moveToStrike(team_obj, ball_coords, 0., [], np.array([np.copysign(1, OP_GOAL[0]), 0]))
                            self.tgts[i] = HighCmd(robot_id=i, skill=0, mod=0, tgt_x=pos[0], tgt_y=pos[1], tgt_theta=0.)
                            """
                            elif ball_coords[0] > FIELD_LIMS[0] - 0.8 or ball_coords[0] < -FIELD_LIMS[0] + 0.8 and not -GOAL_WIDTH-0.5 < ball_coords[1] < GOAL_WIDTH+0.5:
                                print("side margins")
                                self.cmds[i], pos = self.skills.moveToStrike(team_obj, ball_coords, 0., [], np.array([0, -np.copysign(1, team_coords[1])]))
                                self.tgts[i] = HighCmd(robot_id=i, skill=0, mod=0, tgt_x=pos[0], tgt_y=pos[1], tgt_theta=0.)
                            """
